...
first potential with no free parameters, so its $(a,r_0)$ is a prediction.
Forced the unit conversion to be used and exposed that its core is soft.
\textbf{Open:} no published HFD-B value found to check against. Review
point, Sec.~18. \\
...
is not reconstructed here --- replacing one guess with another is how the wrong
entry got written in the first place. Kept visible because a false entry in an
autopsy is worse than a gap in one.
...
\begin{tabular}{lrr}
\toprule
 & Aziz HFD-B (1987) & Table 1 of~\cite{rbef} \\
\midrule
$a$ & $82.98$~\AA & $90.4$~\AA \\
$r_0$ & $7.25$~\AA & $8.0$~\AA \\
$E$ & $-1.93$~mK & $-1.62$~mK \\
...
\textbf{These two columns are not the same potential, and the difference should
not be read as an error.} The numbers in Table 1 of~\cite{rbef} come from
Cencek \emph{et al.}~\cite{cencek}, a 2012 \emph{ab initio} surface. HFD-B is a
1987 fit. A disagreement of $8\%$ in $a$ between them is a statement about the
two potentials, not about this solver.

\textbf{What could not be verified.} A published HFD-B value for $a$ and $r_0$
was not located, so the numbers above stand unchecked against their own source.
The one external number found for comparison is the TTY potential, at
$a = 100.01$~\AA{} and $r_0 = 7.329$~\AA; our $r_0 = 7.25$~\AA{} sits beside it,
which is reassuring about the scale but is not a check. \emph{This is an open
item, not a result.} The test in \texttt{test\_lab.py} verifies only what can
be verified without an external table: that the parameters put the minimum at
$r_m$ with depth $\varepsilon$, which catches a mistyped digit but would not
catch a wrong functional form.
...
Reading the two the other way round is an easy mistake --- it was made while
these notes were being written, and it put the two coefficients under each
other's names in the flat-direction analysis of Sec.~\ref{sec:flat} until it was
caught.}
